...
Two challenges that have not been addressed simultaneously in the
literature are full coverage of 120 syllabic classes and extreme class
imbalance, with an imbalance ratio of 33.10$\times$ (largest class
695 images versus smallest class 21 images) between the Vowel A
classes as the majority and the five other vowel groups.
...
exhibits extreme imbalance: Vowel A averages 500 images per class
...
\textit{shallow learning} methods~\cite{sulistiyo2025}.
...
and achieved F1\,=\,0.979 for Javanese script without any augmentation.
...
an empirical warning that became the primary foundation for the
...
\textit{shallow learning} methods on the same dataset and found that
LightGBM achieved only 77.15\% F1-score, far below CNN-based
approaches, reinforcing the superiority of deep learning for this task.
Ifriza et al.~\cite{ifriza2026} demonstrated that ResNet-18~%
\cite{he2016resnet} fine-tuned with a \textit{transfer learning}
strategy can achieve 95.53\% accuracy from only 1,562 images of
20-class Javanese script, making it the sole ResNet-based
\textit{transfer learning} reference for Javanese script prior to
this study.
...
scenario.
This study fills that gap with a lighter approach --- structure-preserving
...
Widiarti and Adji~\cite{widiarti2026} established a baseline
...
studies~\cite{wisesty2026,sulistiyo2025}.
For the Javanese script subset, the dataset consists of two parts with
a total of 13,257 images: the \texttt{all\_class/} subdirectory
...
All images were converted to 3-channel grayscale and normalized
...
distribution problem: maintaining minority class representation in
...
architecture families: compound scaling efficiency (EfficientNet),
...
ResNet-18~\cite{he2016resnet}                 & $\sim$11.7M & Ifriza et al.~\cite{ifriza2026} \\
...
Top-1 accuracy, macro F1-score, and weighted F1-score as overall
metrics.
...
approximately 1,519 test samples) and the non-A vowel group (100
...
which have only approximately 300 augmented training images --- can
...
Ifriza et al.~\cite{ifriza2026}        & 20  & Yes & Accuracy 95.53\% \\
Wisesty et al.~\cite{wisesty2026}      & 20  & Yes & F1\,=\,0.979   \\
...
One limitation must be acknowledged: each non-A Vowel class is
